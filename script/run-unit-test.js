...
const coveragePath = `NODE_ENV=test nyc --all ${coverageInclude} --reporter=json-summary mocha --reporter mocha-multi-reporters --reporter-options configFile=config/mocha/porter.json --no-color --retries 5`;
const testRunner = options.coverage ? coveragePath : mochaPath;
const configFile = options.config ? options.config : 'config/mocha/mocha.json';
